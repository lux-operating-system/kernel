...
void (*syscallDispatchTable[])(SyscallRequest *) = {
    ...
    syscallDispatchConnect,     // 43 - connect()
    syscallDispatchBind,        // 44 - bind()
    syscallDispatchListen,      // 45 - listen()
    syscallDispatchAccept,      // 46 - accept()
    syscallDispatchRecv,        // 47 - recv()
    syscallDispatchSend,        // 48 - send()
    syscallDispatchKill,        // 49 - kill()
    syscallDispatchSigAction,   // 50 - sigaction()
    syscallDispatchSigreturn,   // 51 - sigreturn()

    /* group 4: memory management */
    syscallDispatchSBrk,        // 52 - sbrk()
    syscallDispatchMmap,        // 53 - mmap()
    syscallDispatchMunmap,      // 54 - munmap()
    syscallDispatchMsync,       // 55 - msync()

    /* group 5: driver I/O functions */
    syscallDispatchIoperm,      // 56 - ioperm()
    syscallDispatchIRQ,         // 57 - irq()
    syscallDispatchIoctl,       // 58 - ioctl()
    syscallDispatchMMIO,        // 59 - mmio()
    syscallDispatchPContig,     // 60 - pcontig()
    syscallDispatchVToP,        // 61 - vtop()
};
